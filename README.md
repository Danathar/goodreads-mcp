...
L4 is deliberately short of autonomy: all agents may file issues; the quality, security and CI agents may additionally open pull requests carrying a hold label; every other agent stays advisory. **A human reviews and merges everything** — nothing reaches `main` without a person having read it. GitHub enforces that only once the committed ruleset is applied; [docs/branch-protection.md](docs/branch-protection.md) says how to check. [docs/SECURITY-AI.md](docs/SECURITY-AI.md) sets out what agents may and may not touch, and the prompt-injection surface that comes with parsing an unofficial third party's payloads.
