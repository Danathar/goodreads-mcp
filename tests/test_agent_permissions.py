...
def test_no_allow_rule_reaches_a_redirection_written_after_a_group():
    ...
    row that allows everything (`Bash`, `Bash(*)`). A wildcard row whose fixed
    part opens a compound (`Bash(if true *)`) matches such a string the same
    way. This fails if a row like that is added; `_reaches_a_group()` says what
    counts (aurora-zfs-simple#241).
    """
    entries = json.loads(_SETTINGS.read_text(encoding="utf-8"))["permissions"]["allow"]
    rules = _bash_allow_rules()
    assert rules
...
def test_the_group_check_catches_a_row_that_reaches_a_group(pattern):
...
def test_the_group_check_leaves_a_plain_row_alone(pattern):
